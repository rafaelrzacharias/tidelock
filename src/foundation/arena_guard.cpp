...
// NON-det half on purpose: guard_tick_begin calls tl_alloc_shim_anchor from alloc_shim (the
// link pull that arms the new/delete tripwires for every guard user), an upward symbol the det
// audit forbids (docs/CPP-SUBSET.md §4); the guard is dev-only engine-side tooling, never
// sim-called. The CRT-delta check was dropped by ruling 2026-08-26 (docs/MEMORY.md §2). Enforcement is one notch stronger than the §8.4 pseudocode:
...
void guard_tick_begin(ArenaGuard* g, const ArenaRegistry* r) {
    ...
    tl_alloc_shim_anchor();   // arms the shim's tripwires in this binary (docs/MEMORY.md §2)
    g->in_barrier = 0u;
