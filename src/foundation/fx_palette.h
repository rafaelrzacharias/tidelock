...
// --- the rows (docs/CANON.md "The fx palette", rev 1.1) ------------------------------------
...
using omega_t   = fx<i32, 20>;   // +-2,048 turn/s
...
static_assert((i64(1) << omega_t::INT_BITS)   >= 2048,      "omega_t: +-2,048 turn/s");
...
// The INV_H product pos_t x 480 -> vel_t is a 2-bit widening: exact by construction.
static_assert(vel_t::FRAC_BITS - pos_t::FRAC_BITS == 2 && omega_t::FRAC_BITS - angle_t::FRAC_BITS == -10, "");
...
// (pos_t == invmass_t, vel_t == omega_t, q_t == stiff_t == angle_t == dt_t, scalar_t == lambda_t),
// so the compiler checks scale, never units, and a line below serves every row of its formats.
...
// angular velocity:      omega = mul_int<omega_t>(wrap_sub(theta, ptheta), INV_H)   (narrow 10)
FX_OP(omega_t,     angle_t,   i32)
// rotate:                theta += mul<angle_t>(omega, H)                          (shift 20)
...
FX_OP(vel_t,       q_t,       vel_t)        // also omega_t
...
FX_OP(vel_t,       scalar_t,  vel_t)        // also omega_t; pos_t/invmass_t are the lambda lines
...
FX_OP(q_t,         vel_t,     vel_t)        // also omega_t
...
FX_OP(vel_t,       vel_t,     vel_t)        // also omega_t
