...
**Done (v0):** steps 1–4 green on Windows and Linux x86-64; the CI grep for OS/SDL headers outside `src/platform/` passes; `tl_tests` runs the whole suite against headless in < 5 s; Pi aarch64 cross-build links (runtime verification is the `TESTING.md` §4 bench job).
