...
`vendor/VERSIONS`' row for that lib (RULED 2026-08-26, Rafael —
option (a), bless as written; the record is the `TODO.md` filing under `## Ruling requests`.
Stated per review round 2 N2, after the W2 vendor lane's FreeType
`builds/<platform>/ftsystem.c` patch — FreeType's own platform-customization seam, since
SDL_ttf's `TTF_Init()` gives no runtime hook to inject a custom `FT_Memory` — shipped as a
declared deviation before the sentence existed to authorize it).
...
| **`session_fingerprint`** | at init, once the world is built (`app/` after all registrations) | `build_id` ‖ reflection field tables in registration order — components then event types, per `ECS.md` §10.2 (name-hash, kind, offset, size per field; table name-hash per table) ‖ arena registry order (ids) ‖ action map (name-hash, kind, class per action in order) ‖ `hash(DataTables)` ‖ every `SIM`-flagged cvar value |
