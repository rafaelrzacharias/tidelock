...
| `net/wire.h` | Every `TL_WIRE_STRUCT` of §20.2 + LE read/write pairs + varint/zigzag helpers. `MAX_PEERS`'s C++ home is `core/input.h` (RULED 2026-08-26, Rafael, `TODO.md` RR-24: CANON.md names no header, so core owns it as the seam's consumer-facing side); `wire.h` includes `core/input.h` for it — a scoped exception, that one line plus the include, nothing else about the `InputFrame`/`WireFrame` handoff (still Phase 2's, RR-17) | all |
